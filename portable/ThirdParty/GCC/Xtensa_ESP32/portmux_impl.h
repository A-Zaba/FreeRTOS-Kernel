 ...
 * template is only used to generate the
 * vPortCPUAcquireMutexIntsDisabledInternal and
 * vPortCPUReleaseMutexIntsDisabledInternal functions, which use S32C1 to do an
 * atomic compare & swap. When external memory is used the functions
 * vPortCPUAcquireMutexIntsDisabledExtram and
 * vPortCPUReleaseMutexIntsDisabledExtram are also generated, which use
 * uxPortCompareSetExtram to fake the S32C1 instruction. The wrapper functions
 * vPortCPUAcquireMutexIntsDisabled and vPortCPUReleaseMutexIntsDisabled will
 * then use the appropriate function to do the actual lock/unlock.
 */
#include "portable.h"
#include "soc/cpu.h"
...
#undef PORTMUX_COMPARE_SET_FN_NAME
    ...
    #define PORTMUX_AQUIRE_MUX_FN_ARGS \
        portMUX_TYPE *mux, int timeout_cycles, const char *fnName, int line
    #define PORTMUX_RELEASE_MUX_FN_ARGS \
        portMUX_TYPE *mux, const char *fnName, int line
...
vPortCPUAcquireMutexIntsDisabled( PORTMUX_AQUIRE_MUX_FN_ARGS )
{
#if defined( CONFIG_SPIRAM_SUPPORT )
    if( esp_ptr_external_ram( mux ) )
    {
        return vPortCPUAcquireMutexIntsDisabledExtram(
            PORTMUX_AQUIRE_MUX_FN_CALL_ARGS( mux ) );
    }
#endif
    return vPortCPUAcquireMutexIntsDisabledInternal(
        PORTMUX_AQUIRE_MUX_FN_CALL_ARGS( mux ) );
}

static inline void vPortCPUReleaseMutexIntsDisabled(
    PORTMUX_RELEASE_MUX_FN_ARGS )
{
#if defined( CONFIG_SPIRAM_SUPPORT )
    if( esp_ptr_external_ram( mux ) )
    {
