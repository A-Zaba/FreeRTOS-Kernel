...
extern void _xt_tick_divisor_init( void );
        ...
        #define SYSTICK_INTR_ID \
            ( ETS_INTERNAL_TIMER0_INTR_SOURCE + ETS_INTERNAL_INTR_SOURCE_OFF )
    #endif
    #ifdef CONFIG_FREERTOS_CORETIMER_1
        #define SYSTICK_INTR_ID \
            ( ETS_INTERNAL_TIMER1_INTR_SOURCE + ETS_INTERNAL_INTR_SOURCE_OFF )
    #endif
...
_Static_assert( SOC_CPU_CORES_NUM <= SOC_SYSTIMER_ALARM_NUM - 1,
                "the number of cores must match the number of core alarms in "
                "SYSTIMER" );
 ...
 * It is done at the same time so SysTicks for both CPUs occur at the same time
 * or very close. Shifts a time of triggering interrupts for core 0 and core 1.
 */
void vPortSetupTimer( void )
{
    ...
    ESP_ERROR_CHECK(
        esp_intr_alloc( ETS_SYSTIMER_TARGET0_EDGE_INTR_SOURCE + cpuid,
                        ESP_INTR_FLAG_IRAM | level,
                        SysTickIsrHandler,
                        &systimer_hal,
                        NULL ) );
    ...
    if( cpuid == 0 )
    {
        ...
        for( cpuid = 0; cpuid < portNUM_PROCESSORS; ++cpuid )
        {
            ...
            if( cpuid == 0 )
            {
                ...
                systimer_hal_enable_counter( &systimer_hal,
                                             SYSTIMER_LL_COUNTER_OS_TICK );
    #ifndef CONFIG_FREERTOS_UNICORE
                // SysTick of core 0 and core 1 are shifted by half of period
                systimer_hal_counter_value_advance( &systimer_hal,
                                                    SYSTIMER_LL_COUNTER_OS_TICK,
                                                    1000000UL /
                                                        CONFIG_FREERTOS_HZ /
                                                        2 );
 ...
 *
 * The Systimer interrupt for SysTick works in periodic mode no need to calc the
 * next alarm. If a timer interrupt is ever serviced more than one tick late, it
 * is necessary to process multiple ticks.
 */
IRAM_ATTR void SysTickIsrHandler( void * arg )
{
    ...
    do
    {
        ...
        uint32_t diff = systimer_hal_get_counter_value(
                            systimer_hal,
                            SYSTIMER_LL_COUNTER_OS_TICK ) /
                            systimer_ll_get_alarm_period( systimer_hal->dev,
                                                          alarm_id ) -
                        s_handled_systicks[ cpuid ];
        if( diff > 0 )
        {
