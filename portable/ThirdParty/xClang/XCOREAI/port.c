...
static int prvCoreInit( void )
{
    ...
    if( xCoreID == 0 )
    {
        ...
        triggerable_setup_interrupt_callback( xKernelTimer,
                                              NULL,
                                              RTOS_INTERRUPT_CALLBACK(
                                                  pxKernelTimerISR ) );
...
DEFINE_RTOS_KERNEL_ENTRY( void, vPortStartSchedulerOnCore, void )
{
    ...
    asm volatile( "mov r6, %0\n\t" /* R6 must be the FreeRTOS core ID*/
                  "ldaw r5, dp[pxCurrentTCBs]\n\t" /* R5 must be the TCB list
                                                      which is indexed by R6 */
