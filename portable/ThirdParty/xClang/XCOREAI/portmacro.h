...
#define PORTMACRO_H
     ...
     * By default, FreeRTOS will require standard time.h, where clock is a
     * function.
     */
...
typedef uint32_t TickType_t;
        ...
        /* 32-bit tick type on a 32-bit architecture, so reads of the tick count
...
#define portTHREAD_CONTEXT_STACK_GROWTH RTOS_SUPPORT_INTERRUPT_STACK_GROWTH
    ...
    /* Check validity of number of cores specified in config */
         ...
         portMAX_CORE_COUNT < configNUMBER_OF_CORES )
        #error "Invalid number of cores specified in config!"
    #endif

    #define portMEMORY_BARRIER() RTOS_MEMORY_BARRIER()
    #define portTASK_STACK_DEPTH( pxTaskCode ) \
        RTOS_THREAD_STACK_SIZE( pxTaskCode )
    /*-----------------------------------------------------------*/
...
void vPortYieldOtherCore( int xOtherCoreID );
    ...
    /* Architecture specific optimisations. */
        ...
        #define portGET_HIGHEST_PRIORITY( uxTopPriority, uxReadyPriorities ) \
            uxTopPriority = ( 31UL - ( uint32_t ) __builtin_clz(             \
                                         uxReadyPriorities ) )
    ...
    #define portCLEAR_INTERRUPT_MASK( ulState ) \
        rtos_interrupt_mask_set( ulState )
...
int xscope_gettime( void );
        #define portCONFIGURE_TIMER_FOR_RUN_TIME_STATS()    /* nothing needed \
                                                               here */
    ...
    #define portTIMER_CALLBACK_ATTRIBUTE \
        __attribute__( ( fptrgroup( "timerCallbackGroup" ) ) )

    /* Timer callback function macros. For xcc this ensures they get added to
    the timer callback group so that stack usage for certain functions in
    timers.c can be calculated. */
    #define portTIMER_CALLBACK_FUNCTION_PROTO( vFunction, xTimer ) \
        void vFunction( TimerHandle_t xTimer )
    #define portTIMER_CALLBACK_FUNCTION( vFunction, xTimer ) \
        portTIMER_CALLBACK_ATTRIBUTE void vFunction( TimerHandle_t xTimer )

    /*-----------------------------------------------------------*/

    /* Task function macros as described on the FreeRTOS.org WEB site.  These
    are not necessary for to use this port.  They are defined so the common demo
